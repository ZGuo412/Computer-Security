...
def x931(v0, dt, key_file, totalNum):
    ...
    while num < totalNum:
        ...
        ede1 = bv.read_bits_from_file(128)  #get the bitvector from first ede
        aes(ede1 ^ v0, key_words, two, three, modulus)
        bv = BitVector(filename='temp.txt')
        ede2 = bv.read_bits_from_file(128)  #get the random number by the second ede
        aes(ede1 ^ ede2, key_words, two, three, modulus)
        bv = BitVector(filename='temp.txt')
        v0 = bv.read_bits_from_file(128)   #update the variable that can be used for next random variable generation
        ran.append(int(ede2))
        num += 1
    return ran
